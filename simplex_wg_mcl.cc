...
MonolithicWorkgroupSimplexCLSolver::MonolithicWorkgroupSimplexCLSolver() {
    problemBuf = 0;
    tableauBuf0 = 0;
    tableauBuf1 = 0;
    indexBuf = 0;
    ...
    cmdQ = cl.createQueue();
    program = cl.buildProgram(loadTextFile("cl/monolithic_wg.cl"));
...
bool MonolithicWorkgroupSimplexCLSolver::initOK() const {
    return cmdQ && program && setupTableau && simplex;
}

void MonolithicWorkgroupSimplexCLSolver::printStats() const {
...
string MonolithicWorkgroupSimplexCLSolver::report() const {
    stringstream ss;
...
bool MonolithicWorkgroupSimplexCLSolver::init(Matrix& problem) {
    steps = 0;
...
void MonolithicWorkgroupSimplexCLSolver::solve(Matrix& problem) {
    if (verbose) {
...
bool MonolithicWorkgroupSimplexCLSolver::buildBuffers(Matrix& problem) {
    CLSys& cl = CLSys::getInstance();
...
bool MonolithicWorkgroupSimplexCLSolver::runSetup(const Matrix& problem) {
    cl_int error = CL_SUCCESS;
...
bool MonolithicWorkgroupSimplexCLSolver::runSimplex(const Matrix& problem) {
    cl_int error = CL_SUCCESS;

    const uint32_t tableSize[2] = { problem.rows(), problem.rows() + problem.columns() };
    const uint32_t partition[2] = { rowPartition, colPartition };
    ...
    clSetKernelArg(simplex, 3, sizeof(int32_t) * 2, &tableSize);
    clSetKernelArg(simplex, 4, sizeof(int32_t) * 2, &partition);
    clSetKernelArg(simplex, 5, sizeof(int32_t), &maxSteps);
    clSetKernelArg(simplex, 6, sizeof(uint32_t), &variables);

    const size_t workGroupSize = std::min(CLSys::getInstance().deviceWorkGroupSize(), (size_t)(tableSize[0] * tableSize[1]));
    cl_event simplex_complete;
    error = clEnqueueNDRangeKernel(cmdQ, simplex, 1, nullptr, &workGroupSize, nullptr, lastEvent ? 1 : 0, lastEvent ? &lastEvent : 0, &simplex_complete);
...
bool MonolithicWorkgroupSimplexCLSolver::extractSolution(const Matrix& problem) {
    cl_int error = CL_SUCCESS;
    int32_t steps_abort_pivot[4];
    error = clEnqueueReadBuffer(cmdQ, indexBuf, CL_TRUE, 0, 4 * sizeof(int32_t), steps_abort_pivot, lastEvent ? 1 : 0, lastEvent ? &lastEvent : 0, nullptr);
    E_CL(error, "R(steps/abort)");
    lastEvent = nullptr;
    steps = steps_abort_pivot[0];
    aborted = steps_abort_pivot[1] == 1;
    noSolution = steps_abort_pivot[1] == 2;

    if (verbose) {
        int32_t* p = steps_abort_pivot + 2;
        cout << "MCL-Solver:: last pivot @ [" << p[0] << ", " << p[1] << "]\n";
    }
...
void MonolithicWorkgroupSimplexCLSolver::showBuffer(const cl_mem& buffer, int rows, int columns) {
